...
Two console commands repair subscriptions and transactions that a bug left in a bad state — subscriptions stuck as `pending` without a `subscriptionId`, and paid transactions without a `paidAt`. See [Console commands](/console-commands).
